...
# This function adds a base plot to a figure
...
def min_distance_plot(df, output_name, mode):
    if mode == 'cummulative':
        x = df
        binwidth = 20
        counts, bins = np.histogram(x, bins=range(0, 500 + binwidth, binwidth))
        ...
        plt.xticks(np.arange(min(x), 520, 100), rotation=30)
        ...
        dist = df
        binwidth = 20
        counts, bins = np.histogram(x, bins=range(0, int(max(dist)) + binwidth, binwidth))
        cdf = np.cumsum(counts) / np.sum(counts) * 100
        plt.subplots(figsize=(8, 5))
        plt.plot(
            np.vstack((bins, np.roll(bins, -1))).T.flatten()[:-2],
        ...
        plt.grid(axis='y', alpha=0.5)
        plt.xticks(np.arange(min(dist), max(dist), 100), rotation=30)
...
# Create histogram of automatic %LF values
def hist_automatic():
    ...
    m = plt.Rectangle((0.01, 0), 0.67, 100, fc='#E8F5E9', alpha=alpha)
    l = plt.Rectangle((0.68, 0), 0.4075, 100, fc='#81C784', alpha=alpha)
    xl = plt.Rectangle((1.0875, 0), 17, 100, fc='#43A047', alpha=alpha)
    #ax.add_patch(s)
    ax.add_patch(m)
    ax.add_patch(l)
    ax.add_patch(xl)
...
# This function plots KDEs for each subtype of CES
